...
function cerrarSesion() { passwordGuardada = ""; todosLosRegistros = []; window.location.href = "/"; }
...
window.toggleSidebar = function() {
    ...
    else { sb.style.right = '0px'; cargarSolicitudesPendientes(); }
}
...
async function cargarSolicitudesPendientes() {
    ...
    try {
        ...
        if (solicitudes.length === 0) { contenedor.innerHTML = '<p style="color:#999;">No hay solicitudes.</p>'; return; }
        contenedor.innerHTML = '';
        solicitudes.forEach(sol => {
            const nuevos = sol.nuevos_datos; const original = sol;
            ...
            card.innerHTML = `<div style="font-weight:bold; color:#005696;">${nuevos.nombre}</div>
                <div style="font-size:0.85em; margin-bottom:5px;">${cambioInst}${cambioUbi}Tipo: <b>${nuevos.tipoEnlace||'N/A'}</b><br>${nuevos.puesto} - ${nuevos.unidad}<br>${nuevos.correo}<br>${nuevos.celular}</div>
                <div style="display:flex; gap:5px;"><button onclick='aprobarEdicion(${JSON.stringify(sol)})' style="flex:1; background:#28a745; color:white; border:none; padding:5px; border-radius:4px;">✔</button><button onclick="rechazarEdicion(${sol.solicitud_id})" style="flex:1; background:#dc3545; color:white; border:none; padding:5px; border-radius:4px;">✕</button></div>`;
...
async function aprobarEdicion(solicitud) {
    ...
    try {
        const res = await fetch('/api/ediciones', { method: 'PUT', headers: { 'x-admin-password': passwordGuardada, 'Content-Type': 'application/json' }, body: JSON.stringify({ solicitud_id: solicitud.solicitud_id, registro_id: solicitud.id, datos_finales: solicitud.nuevos_datos }) });
        if(res.ok) { Swal.fire('Aprobado', 'Listo', 'success'); cargarSolicitudesPendientes(); checkLogin(); }
    } catch(e) {}
}
async function rechazarEdicion(id) { if(confirm('¿Rechazar?')) await fetch(`/api/ediciones?solicitud_id=${id}`, { method: 'DELETE', headers: { 'x-admin-password': passwordGuardada } }); cargarSolicitudesPendientes(); }

// --- EXCEL SEPARADO ---
function exportarExcelUnico(inst) {
    const data = [["ID", "Nombre", "Puesto", "Unidad", "Tipo Enlace", "Departamento", "Municipio", "Correo", "Celular"]];
    todosLosRegistros.filter(r => r.institucion === inst).forEach(r => data.push([r.id, r.nombre_completo, r.puesto, r.unidad_direccion, r.tipo_enlace||'', r.departamento||'', r.municipio||'', r.correo||'', r.celular||'']));
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(data), "Datos");
...
function exportarTodoGlobal() {
    ...
    let datos = []; const headers = ["ID", "Nombre", "Puesto", "Unidad", "Tipo Enlace", "Departamento", "Municipio", "Correo", "Celular"];
    ...
    instituciones.forEach(inst => {
        ...
        todosLosRegistros.filter(r => r.institucion === inst).forEach(r => datos.push([r.id, r.nombre_completo, r.puesto, r.unidad_direccion, r.tipo_enlace, r.departamento, r.municipio, r.correo, r.celular]));
        datos.push([]); datos.push([]);
    });
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(datos), "Reporte Completo");
...
function cargarRegistrosPorInstitucion(registros) {
    ...
    instituciones.forEach(inst => {
        ...
        const details = document.createElement('details'); details.className = 'lista-desplegable';
        details.innerHTML = `<summary><span>${inst} <span style="font-weight:normal; opacity:0.8;">(${registrosInst.length})</span></span><button onclick="exportarExcelUnico('${inst}')" class="btn-excel" style="margin-left:auto;">📥 Excel</button></summary>
            <div class="tabla-responsive"><table id="tabla-${inst.replace(/\s/g, '-')}" border="1"><thead><tr><th>ID</th><th>Nombre</th><th>Puesto</th><th>Unidad</th><th>Tipo Enlace</th><th>Ubicación</th><th>Contacto</th><th>Acciones</th></tr></thead><tbody>
            ${registrosInst.map(r => `<tr><td>${r.id}</td><td>${r.nombre_completo}</td><td>${r.puesto}</td><td>${r.unidad_direccion}</td><td style="color:#005696; font-weight:bold;">${r.tipo_enlace || '-'}</td><td>${r.departamento}${r.municipio ? ' - '+r.municipio : ''}</td><td>${r.correo}<br>${r.celular}</td><td style="text-align:center;"><button onclick="borrarRegistro(${r.id})" style="background:#dc3545;color:white;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;">Borrar</button></td></tr>`).join('')}
            </tbody></table></div>`;
